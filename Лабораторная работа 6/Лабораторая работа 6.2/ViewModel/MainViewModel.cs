...
namespace Лабораторая_работа_6._2.ViewModel
{
    public partial class MainViewModel : ObservableObject
    {
        ...
        private List<CancellationTokenSource> _cancellationTokenSourceList = new();
        ...
        private void BubbleSort()
        {
            ...
            new Thread(() =>
            {
                ...
                {
                    _cancellationTokenSourceList.Remove(_cancellationTokenSource);
                }
            })
            ...
            new Thread(() =>
            {
                ...
                try
                {
                    threads.Add(new Thread(() =>
                    {
                        try
                        {
                            _sorter.BubbleSort(_originalArray, token);
                        }
                        catch (OperationCanceledException)
                        {
                            _uiContext.Post(_ =>
                    ...
                    threads.Add(new Thread(() =>
                    {
                        try
                        {
                            _sorter.QuickSort(_originalArray, token);
                        }
                        catch (OperationCanceledException)
                        {
                    ...
                    threads.Add(new Thread(() =>
                    {
                        try
                        {
                            _sorter.InsertionSort(_originalArray, token);
                        }
                        catch (OperationCanceledException)
                        {
                    ...
                    threads.Add(new Thread(() =>
                    {
                        try
                        {
                            _sorter.MergeSort(_originalArray, token);
                        }
                        catch (OperationCanceledException)
                        {
        ...
        {
            if (_cancellationTokenSourceList.Count != 0 && !_isCancelling)
            {
                //_isCancelling = true;
                foreach (var token in _cancellationTokenSourceList)
                    token.Cancel();
                CancelSortingCommand.NotifyCanExecuteChanged();
                SortAllCommand.NotifyCanExecuteChanged();
            }
        ...
        {
            TotalComparisonsAsync = "[ASYNC] Выполняется...";
            TotalExecutionTimeAsync = "[ASYNC] Выполняется...";

            SortAllAsyncCommand.NotifyCanExecuteChanged();
            CancelSortingCommand.NotifyCanExecuteChanged();

            var bubbleTask = BubbleSortAsyncRun();
            var quickTask = QuickSortAsyncRun();
            var insertionTask = InsertionSortAsyncRun(); ;
            var mergeTask = MergeSortAsyncRun();
